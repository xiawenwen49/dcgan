...
if __name__ == '__main__':
    ...
    if dataset == 'mnist':
        ...
        train_iter = ImagenetIter(imgnet_pathR, batch_size, (3, 64, 64))
        train_iterS = ImagenetIter(imgnet_pathS, batch_size, (3, 64, 64))
        train_iterVA = ImagenetIter(imgnet_pathVR, batch_size, (3, 64, 64))
        train_iterVS = ImagenetIter(imgnet_pathVS, batch_size, (3, 64, 64))
    ...
    if load is True:
        modG.load_params('imagenet_G_2017_05_03-20_41-0004.params')
        modD.load_params('imagenet_D_2017_05_03-20_41-0004.params')
        print("load is finished")


        # for t in range(5):
        for t, batch in enumerate(train_iterVS):
    ...
    for epoch in range(5):
        ...
        train_iterS.reset()
        t = 0
        for batch, batchS in zip(train_iter, train_iterS):
            rbatch = rand_iter.next()
            ...
            label[:] = 0
            modD.forward(mx.io.DataBatch(outG, [label]), is_train=True) # compute predictions
            modD.backward()  # compute gradients
            modD.update()

            modD.update_metric(mD, [label])
            modD.update_metric(mACC, [label])

            # update discrimimator on spliced images

            # for tS, batchS in enumerate(train_iterS):
            #     batchS.label = [label]
            #     modD.forward(batchS, is_train=True)
            #     modD.backward()
            ...
            # gradD actually is all the gradiants of module D
            ...
            # 把gradD加到这次的梯度上，一起更新。gradD：对于fake image应该调整的梯度。这次是对于real image应该调整的梯度。
            ...
            # result.write("%d %d %f %f %f\n " % (epoch, t, mACC.get()[1], mG.get()[1], mD.get()[1]))
            if t % 10 == 0:
                # print('epoch:', epoch, 'iter:', t, 'metric:', mACC.get(), mG.get(), mD.get())
                mACC.reset()
                mG.reset()
